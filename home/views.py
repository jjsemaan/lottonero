...
def index(request):
    """
    A view to return the index page with the latest draw results and latest
    winning predictions.

    This view fetches the latest EuroMillions draw result from the database.
    It also identifies the latest prediction date where predictions have a
    non-null match type and fetches all predictions for that date.
    Additionally, it fetches all winning predictions. The context containing
    the latest draw result, latest predictions, and winning predictions is
    then passed to the 'index.html' template for rendering.

    Args:
        request: The HTTP request object.

    Returns:
        HttpResponse: The rendered 'index.html' template with the context
        containing latest_result,
    ...
    total_winning_amount_predictions = (
        Prediction.objects.filter(win_amount__isnull=False).aggregate(
            Sum("win_amount")
        )["win_amount__sum"]
        or 0
    )

    total_winning_amount_shuffled = (
        ShuffledPrediction.objects.filter(win_amount__isnull=False).aggregate(
            Sum("win_amount")
        )["win_amount__sum"]
        or 0
    )
    ...
    total_combined_winning_amount = (
        total_winning_amount_predictions + total_winning_amount_shuffled
    )
...
def get_total_winning_amount(predictions):
    ...
    """
    return sum(
        prediction.win_amount
        for prediction in predictions
        if prediction.win_amount
    )


def alltime_winning_predictions_view(request):
    """
    Serves as the view for displaying all-time winning predictions with
    pagination.

    This view fetches all predictions from the database that have a non-null
    match type, indicating that they are winning predictions. It calculates the
    total winning amount from all such predictions and allows viewing these
    predictions in a paginated format.
    If accessed via an AJAX request, it returns only the HTML for the
    predictions list, suitable for dynamic page updates.

    Args:
        request (HttpRequest): The HTTP request object, which can contain a
        GET parameter 'num_draws' specifying the number of predictions to
        display per page (defaults to 25).
        This can also handle AJAX requests for dynamic pagination without
        reloading the page.

    Returns:
        HttpResponse: Renders the 'alltime.html' template with the paginated
        winning predictions and the total winning amount, or a JsonResponse
        containing HTML content if it's an AJAX request.
    ...
    total_winning_amount = (
        alltime_winning_predictions.aggregate(total=Sum("win_amount"))["total"]
        or 0
    )
    ...
    if request.headers.get("X-Requested-With") == "XMLHttpRequest":
        html = render_to_string(
            "partial_predictions_list.html", context, request
        )
...
def latest_predictions_with_matches(request):
    """
    A view to update the latest predictions with matching results
    against the latest EuroMillions draw, ensuring that updates
    are only performed if the prediction
    ...
    Returns:
        redirect: Redirects to the previous page with appropriate
        messages depending on the action outcome.
    """
    ...
    if request.method == "POST":
        ...
        if latest_date != datetime.today().date():
            messages.error(
                request, "Winning predicted patterns already checked!"
            )
        ...
        messages.success(
            request, "Match results updated. Check the homepage for details."
        )
    else:
...
def format_numbers(numbers):
    """
    Format a set of numbers as a comma-separated string, without extra
    commas for single numbers.
    """
...
def latest_shuffled_predictions_with_matches(request):
    """
    A view to update the latest shuffled predictions with matching results
    against the latest EuroMillions draw,
    ...
    if request.method == "POST":
        ...
        if latest_date != datetime.today().date():
            messages.error(
                request, "Winning predicted combinations already checked!"
            )
            return redirect(request.META.get("HTTP_REFERER", "/"))
        ...
        messages.success(
            request, "Match results updated. Check the database for details."
        )
    else:
        messages.error(request, "This endpoint only accepts POST requests.")
...
def alltime_winning_shuffled_predictions_view(request):
    """
    View function to display all-time winning shuffled predictions with their
    associated images.

    This view retrieves shuffled predictions from the database where each
    prediction has a non-null match type and a winning amount
    greater than zero.
    It calculates the total winning amount across all such predictions and
    supports pagination for displaying these records.

    Args:
        request (HttpRequest): The request object used to fetch the number
        of records (`num_draws`) to display per page from GET parameters.
        Defaults to 25 records per page if not specified. If 'all' is
        specified, all records are displayed on one page.

    Returns:
        HttpResponse: Renders the 'alltime_shuffled.html' template with
        context containing:
            - alltime_winning_shuffled_predictions:
            A paginated list of predictions with additional data for displaying
            associated images.
            - total_winning_amount:
            The sum of win amounts across all eligible shuffled predictions.
    """
    num_draws = request.GET.get("num_draws", "25")

    base_query = ShuffledPrediction.objects.filter(
        match_type__isnull=False, win_amount__isnull=False, win_amount__gt=0
    )

    total_winning_amount = (
        base_query.aggregate(total=Sum("win_amount"))["total"] or 0
    )
    ...
    if num_draws != "all":
        paginator = Paginator(
            alltime_winning_shuffled_predictions, int(num_draws)
        )
        page_number = request.GET.get("page", 1)
    ...
    for prediction in alltime_winning_shuffled_predictions:
        pred_ball_images = [
            (
                get_image_url(
                    f"green{getattr(prediction, f'pred_ball_{i}'):02}"
                )
                if getattr(prediction, f"pred_ball_{i}")
                in map(
                    int,
                    (
                        prediction.winning_balls.split(",")
                        if prediction.winning_balls
                        else []
                    ),
                )
                else get_image_url(
                    f"{getattr(prediction, f'pred_ball_{i}'):02}"
                )
            )
            for i in range(1, 6)
        ]
        ...
        pred_lucky_images = [
            (
                get_image_url(
                    f"greenstar{getattr(prediction, f'pred_lucky_{i}')}"
                )
                if getattr(prediction, f"pred_lucky_{i}")
                in map(
                    int,
                    (
                        prediction.winning_lucky_stars.split(",")
                        if prediction.winning_lucky_stars
                        else []
                    ),
                )
